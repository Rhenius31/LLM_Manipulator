...
class MoveGroupExecutor(Node):
    def __init__(self):
        ...
        self.current_task_summary = None
    ...
    def publish_task_result(self, status: str, reason: str = "", extra: dict = None):
        msg = {
            "status": status,
            "reason": reason,
        }
        if self.current_task_summary is not None:
            msg["command"] = self.current_task_summary
        if extra:
            msg.update(extra)
        self.result_pub.publish(String(data=json.dumps(msg)))
    ...
    def cb_plan(self, msg: String):
        ...
        if steps:
            ok = self.execute_symbolic_steps(plan)
            if ok:
                return
            self.publish_task_result("failed", "symbolic_step_error")
            return
        ...
        if plan.get("intent") == "pick_and_place":
            ...
            if pick_obj is None:
                ...
                self.publish_task_result("failed", "pick_object_not_found")
                return
            ...
            if place_on["class"] == "table":
                ...
                if place_obj is None:
                    ...
                    self.publish_task_result("failed", "place_target_not_found")
                    return
                place_pose = dict(place_obj["pose"])

            self.current_task_summary = f"pick {pick['class']} place on {place_on['class']}"
            self.queue_pick_place_from_objects(pick_obj, place_pose)
            return
        
        if plan.get("intent") == "pick_and_hold":
            pick = plan["objects"]["pick"]

            pick_obj = self.select_object(
                pick.get("id"),
                pick["class"],
                int(pick.get("index", 0))
            )
            if pick_obj is None:
                self.get_logger().error(f"No pick object for {pick}")
                self.publish_task_result("failed", "pick_object_not_found")
            ...
            self.current_task_summary = f"hold {pick['class']}"
            self.queue_sequence(self.expand_pick(pick_obj))
            return

        self.get_logger().warn(f"Unsupported intent: {plan.get('intent')}")
        self.publish_task_result("failed", "unsupported_intent")
    ...
    def tick(self):
        ...
        if self.step_idx >= len(self.seq):
            ...
            self.busy = False
            self.seq = []
            self.publish_task_result("success")
            return
    ...
    def abort_sequence(self):
        ...
        self.publish_task_result("failed", "sequence_aborted")
    ...
    def publish_scene_objects_as_collisions(self):
        ...
        for i, obj in enumerate(objs):
            ...
            frame = pose.get("frame", pose.get("frame_id", self.base_frame))
